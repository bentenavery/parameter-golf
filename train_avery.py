"""train_avery.py — PR #462 base (SwiGLU + XSA4 + BigramHash + EMA + TTT + Late-QAT + int6+zstd)
+ Differential Attention (arXiv:2410.05258) on layers 0-6 as novel architectural contribution.

Differential Attention replaces standard softmax attention with:
  Attn = softmax(Q1K1/sqrt(d/2)) - lambda * softmax(Q2K2/sqrt(d/2))
where Q1,K1 = first half of head_dim, Q2,K2 = second half.
Same parameter budget as standard attention — head_dim reorganized, not expanded.
Lambda initialized per layer depth (DIFF TRANSFORMER paper formula).
DiffAttn applied to layers 0-6 (early/mid layers); standard + XSA on layers 7-10.
"""
...
class Hyperparameters:
    ...
    # Differential Attention: apply to first N layers (0 = disabled)
    diff_attn_layers = int(os.environ.get("DIFF_ATTN_LAYERS", "7"))  # layers 0-6
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        # diff_lambda added: small fp32 per-head scalars for differential attention
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear_gate,bigram,skip_gates,diff_lambda",
    ).split(",")
    if pattern
)
...
class DifferentialAttention(nn.Module):
    """
    Microsoft Differential Attention (ICLR 2025, arXiv:2410.05258).

    Two softmax attention maps subtracted:
        Attn = softmax(Q1 K1^T / sqrt(d/2)) - lambda * softmax(Q2 K2^T / sqrt(d/2))
    where Q1,K1 = first half of head_dim, Q2,K2 = second half.
    V retains full head_dim — same output shape as standard attention.

    Parameter count is IDENTICAL to CausalSelfAttention:
      - c_q, c_k, c_v, proj matrices are the same shape (just reorganized semantics)
      - Adds only: diff_lambda (n_heads,) = 8 floats per layer. Negligible.

    Lambda is initialized per-layer using the paper's depth formula:
        lambda_init = 0.8 - 0.6 * exp(-0.3 * layer_idx)
    This gives smaller lambda (more differential correction) in early layers,
    growing toward 0.8 in deep layers (closer to standard attention).

    Applied to layers 0-6 (7 of 11 layers) where early representation
    learning benefits most from noise cancellation via differential maps.
    Layers 7-10 retain standard attention + XSA for final representation mixing.
    """
    ...
    def __init__(
        ...
        if self.head_dim % 4 != 0:
            raise ValueError("head_dim must be divisible by 4 for DifferentialAttention")
        kv_dim = self.num_kv_heads * self.head_dim

        # Same projections as CausalSelfAttention — parameter count unchanged
        self.c_q = CastedLinear(dim, dim, bias=False)
        self.c_k = CastedLinear(dim, kv_dim, bias=False)
        self.c_v = CastedLinear(dim, kv_dim, bias=False)
        self.proj = CastedLinear(dim, dim, bias=False)
        ...
        # QK gain (same as standard attention)
        self.q_gain = nn.Parameter(torch.full((num_heads,), qk_gain_init, dtype=torch.float32))

        # Rotary embeddings applied to full head_dim (then split for Q1/Q2, K1/K2)
        self.rotary = Rotary(self.head_dim, base=rope_base, rope_dims=rope_dims)

        # Learnable lambda per head (fp32, very small — treated as control tensor)
        # Paper formula: lambda_init = 0.8 - 0.6 * exp(-0.3 * layer_idx)
        lambda_init = 0.8 - 0.6 * math.exp(-0.3 * layer_idx)
        self.diff_lambda = nn.Parameter(torch.full((num_heads,), lambda_init, dtype=torch.float32))

        # Compatibility flag (DiffAttn never uses XSA; XSA is on layers 7-10)
        self.use_xsa = False

    def forward(self, x: Tensor) -> Tensor:
        bsz, seqlen, dim = x.shape

        # Project Q, K, V — identical to standard attention
        q = self.c_q(x).reshape(bsz, seqlen, self.num_heads, self.head_dim).transpose(1, 2)
        k = self.c_k(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).transpose(1, 2)
        v = self.c_v(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).transpose(1, 2)

        # QK norm and RoPE on full head_dim (standard pipeline)
        q = F.rms_norm(q, (q.size(-1),))
        k = F.rms_norm(k, (k.size(-1),))
        cos, sin = self.rotary(seqlen, x.device, q.dtype)
        q = apply_rotary_emb(q, cos, sin)
        ...
        # Split Q and K along head_dim into two halves
        # Q1,K1 = first half (indices :half_head_dim)
        # Q2,K2 = second half (indices half_head_dim:)
        # V remains at full head_dim — SDPA reduces (B,H,T,T)@(B,Hkv,T,D_full)
        hd = self.half_head_dim
        q1, q2 = q[..., :hd], q[..., hd:]
        k1, k2 = k[..., :hd], k[..., hd:]

        # Scale = 1/sqrt(half_head_dim) — correct for the split dimension
        diff_scale = float(hd) ** -0.5
        enable_gqa = self.num_kv_heads != self.num_heads

        # Two attention maps: (B, H, T, T) @ V -> (B, H, T, head_dim_full)
        ...
        # Differential combination: subtract scaled second map from first
        # lambda shape: (H,) -> broadcast to (1, H, 1, 1)
        lam = self.diff_lambda.to(dtype=attn1.dtype)[None, :, None, None]
        y = attn1 - lam * attn2  # (B, H, T, head_dim_full)
    ...
    def __init__(
        ...
        if use_diff_attn:
            self.attn = DifferentialAttention(
                ...
                layer_idx=layer_idx, rope_dims=rope_dims,
            )
        else:
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                layer_idx=i,
                ln_scale=ln_scale,
                use_diff_attn=(i < diff_attn_layers),  # DiffAttn on first diff_attn_layers layers
            )
            for i in range(num_layers)
        ])
        # XSA applied to last xsa_last_n layers (these use standard attention)
        if xsa_last_n > 0:
...
def ttt_adapt(
    ...
    """AdamW fine-tune on validation data; all blocks unfrozen unless ttt_freeze_blocks > 0."""
    seq_len = args.train_seq_len
    ...
    if args.ttt_mlp_only:
        for name, p in base_model.named_parameters():
            if 'up_proj' not in name and 'down_proj' not in name and 'scale' not in name and 'bias' not in name:
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        diff_attn_layers=args.diff_attn_layers,
    ).to(device).bfloat16()
    ...
    # Differential LR: encoder vs decoder blocks
    ...
    n_diff_layers = sum(1 for b in base_model.blocks if isinstance(b.attn, DifferentialAttention))
    n_std_layers = sum(1 for b in base_model.blocks if isinstance(b.attn, CausalSelfAttention))
    log0(f"model_params:{n_params} diff_attn_layers:{n_diff_layers} std_attn_layers:{n_std_layers}")
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0("sdp_backends:cudnn=False flash=True mem_efficient=False math=False")
    log0(f"tie_embeddings:{args.tie_embeddings} embed_lr:{token_lr} matrix_lr:{args.matrix_lr} scalar_lr:{args.scalar_lr} decoder_lr_mult:{args.decoder_lr_mult}")
    ...
    log0(f"rope_dims:{args.rope_dims} ln_scale:{args.ln_scale} diff_attn_layers:{args.diff_attn_layers} xsa_layers:{args.xsa_layers}")
    ...
    if max_wallclock_ms is not None:
        estimated_total_steps = min(args.iterations, int(max_wallclock_ms / 30))
